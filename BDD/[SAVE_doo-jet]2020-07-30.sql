...
--
-- Hôte : localhost:3306
-- Généré le : jeu. 30 juil. 2020 à 14:53
...
(SELECT * FROM personne 
 ...
 (Statut = 'Client' and
  ...
  THEN
   ...
   INSERT INTO personne(Statut, Nom, Prenom, Date_Naissance, Telephone, N_Permis, ID_Adresse, N_Securite_Sociale, N_BEES, Contrat, Date_Embauche, Date_Visite_Medicale, Password) VALUES('Client', vnom, vprenom, vdate_naissance, vtelephone, vpermis, LAST_INSERT_ID(), vsecu, vbees, vcontrat, vembauche, vmedical, vpassword);
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `update_adresse` (IN `vid_adresse` INT, IN `vnumero_rue` INT, IN `vrue` CHAR(50), IN `vvoie` CHAR(50))  BEGIN
	UPDATE adresse
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `update_tb_personne_client` (IN `VID_Personne` INT, IN `vnom` CHAR(50), IN `vprenom` CHAR(50), IN `vdate_naissance` DATE, IN `vtelephone` VARCHAR(15), IN `vpermis` VARCHAR(20))  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `update_tb_personne_employe` (IN `VID_Personne` INT, IN `vnom` CHAR(50), IN `vprenom` CHAR(50), IN `vdate_naissance` DATE, IN `vtelephone` VARCHAR(15), IN `vpermis` VARCHAR(20), IN `vsecu` VARCHAR(13), IN `vbees` VARCHAR(20), IN `vcontrat` CHAR(50), IN `vembauche` DATE, IN `vmedical` DATE)  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `update_ville` (IN `vid_adresse` INT, IN `vville` CHAR(50), IN `vcode_postal` VARCHAR(255), IN `vpays` CHAR(50))  BEGIN
...
(1, 'Client', 'Foillard', 'Stanislas', '2000-12-03', '+33783297606', NULL, NULL, NULL, '', NULL, NULL, NULL, 1),
(2, 'Employé', 'Foillard', 'Maxime', '2000-03-25', '+33606060606', NULL, '1234547687', '432543564', '432567883', 'CDI', '2020-04-01', '2020-04-03', 2),
(3, 'Employé', 'Foillard', 'Jean', '1965-02-22', '+33606078606', NULL, '1234547687', '432543564', '432567883', 'CDD', '2020-04-01', '2020-04-03', 3),
...
--
ALTER TABLE `adresse`
  MODIFY `ID_Adresse` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=29;
...
--
ALTER TABLE `ville`
  MODIFY `ID_Ville` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=46343;
