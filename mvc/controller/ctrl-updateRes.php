...
switch ($etape) {
    ...
    case 'et1':
        ...
        if($res == 30 || $res == 60 || $res== 120) {
            $valid = "time_resa";
            // Si la temps de la réservatio est de 30 mins ou 1h ou 2h,
            // on peut faire de réservation.}
        }else{
            // on ne peut pas faire de réservation.
            $feedback .= '<script type="text/javascript">alert("Le temps de réservation doit être égal à 30/60/120 minutes! merci de réessayer.");document.getElementById("time_deb").focus();</script>';
        }
        ...
        if ($dateChoisie <= $now) {
            ...
            $feedback .= '<script>window.location.assign("createreservation1");</script>';
        }
        break;
